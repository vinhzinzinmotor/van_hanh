...
const thanhBang = document.getElementById("thanh-bang");
...
function capNhatLaiSoThuTu() {
    const danhSachHang = thanhBang.getElementsByClassName("hang-du-lieu");
    tổngSốHàngHiệnTại = danhSachHang.length;

    for (let i = 0; i < danhSachHang.length; i++) {
        const oSo = danhSachHang[i].getElementsByTagName("td")[0].getElementsByTagName("span")[0];
        if (oSo) {
            oSo.innerText = i + 1;
        }
    }
